...
function publishAssets({ kibanaDir, forkRemote, assetsBranch, fork, pr, stamp, pngFiles }) {
  ...
  try {
    ...
    // Re-publishing the same run (e.g. to tweak the comment body) leaves the
    // assets identical, so there's nothing to commit. Skip commit+push in that
    // case and still return the prefix so the comment can be rebuilt.
    const pending = run('git', ['-C', worktreeDir, 'status', '--porcelain']);
    if (!pending) {
      info(`Screenshots already published at ${destRel} — skipping push.`);
      return destRel;
    }
...
function buildMarkdown({ summary, marker, checksMd, imageBaseUrl, noBreakages }) {
  ...
  } else if (!noBreakages && (summary.breakages || []).length > 0) {
...
function main() {
  ...
  const md = buildMarkdown({ summary, marker, checksMd, imageBaseUrl, noBreakages: args.noBreakages });
